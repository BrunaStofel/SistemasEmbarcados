...
char nome_arq_dadostxt[15], nome_arq_dadosdat[13];
...
void __fastcall TFSerialPort::BtOpenPortClick(TObject *Sender)
{
    ...
    // Decalaração do ponteiro do arquivo que armazena os dados.
    strcpy(nome_arq_dadostxt, "teste123.txt"); 
    ...
    while(!Terminated)
    {
        ...
        if (buffer1[0] == '#' && buffer1[1] == '$' && buffer1[2] == ':')
        {
            ...
            if(Tatual == 5)
            {
                ...
                strcat(nome_arq_dadostxt, ".txt");  //              strcat(nome_arq_dadosdat, ".dat");
                arq_dadostxt = fopen(nome_arq_dadostxt, write);
//                int creat(nome_arq_dadostxt, int O_WRONLY);
//                 arq_dadostxt = fopen(timeStr+".txt",write);
                // Apresenta a saída.

            }else{ //grava no arquivo
//               fwrite(itens, sizeof(DATA), hourToSec, arq_dadosdat);
                 FSerialPort->Log->Lines->Add("Gravando no txt ");
                 //arq_dadostxt = fopen(nome_arq_dadostxt, append);
